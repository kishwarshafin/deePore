...
from modules.model import EncoderCRNN, AttnDecoderRNN, EncoderRNN
...
def evaluate(train_file, batch_size, gpu_mode, encoder, decoder, max_length=51):
    ...
    confusion_meter = np.zeros((3, 3), dtype=np.int)
...
def train(train_file, batch_size, epoch_limit, file_name, gpu_mode):
    ...
    encoder = EncoderCRNN()
    # encoder = EncoderRNN(batch_size, 102, hidden_size=200, n_layers=3)
    ...
    for epoch in range(epoch_limit):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            for col in range(images.size(2)):
                image_input = images[:, :, col:col+1, :] #.data.long()
                # image_input = image_input.view(images.size(0), -1)
                encoder_output, encoder_hidden = encoder(image_input, encoder_hidden)
                encoder_outputs[col] = encoder_output[0]
...
if __name__ == '__main__':
    ...
    train(FLAGS.train_file, FLAGS.batch_size, FLAGS.epoch_size, FLAGS.model_out, FLAGS.gpu_mode)
